...
void cal_barycentric(mat4 MVP, Image image, Sphere sphere, const Light& light, const Camera& camera, mat4 Mm) {
    ...
    // Precompute vertex colors for Gouraud shading
    std::vector<vec3> vertexColors(gNumVertices);
    for (int i = 0; i < gNumVertices; i++) {
        vec3 worldPos = vec3(Mm * vec4(gVertexBuffer[i], 1.0f));
        vec3 normal = normalize(worldPos - sphere.center);
        ...
        vertexColors[i] = ambient + diffuse + specular;
        vertexColors[i] = pow(vertexColors[i], vec3(1.0f / gamma_correct)); // Gamma correction
    }
    ...
    for (int i = 0; i < gNumTriangles; i++) {
        ...
        vec3 c0 = vertexColors[k0];
        vec3 c1 = vertexColors[k1];
        vec3 c2 = vertexColors[k2];
