...
void cal_barycentric(mat4 MVP, Image image, Sphere sphere, const Light& light, const Camera& camera, mat4 Mm) {
    ...
    for (int i = 0; i < gNumTriangles; i++) {
        ...
        vec3 w0 = vec3(Mm * vec4(gVertexBuffer[k0], 1.0f));
        vec3 w1 = vec3(Mm * vec4(gVertexBuffer[k1], 1.0f));
        vec3 w2 = vec3(Mm * vec4(gVertexBuffer[k2], 1.0f));

        vec3 position = (w0 + w1 + w2) / 3.0f;
        ...
        // Gamma correction
        color = pow(color, vec3(1.0f / gamma_correct));
...
void render() {
    ...
    //Baycentric algoritm with flat shading
